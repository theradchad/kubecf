...
wait_for_router() {
  local timeout="1800"
  until [[ "$(bazel run @kubectl//:kubectl -- get endpoints --namespace "${KUBECF_NAMESPACE}" scf-router-public --output jsonpath='{.subsets[0].addresses[0].ip}' 2> /dev/null)" != "" ]] || [[ "$timeout" == "0" ]]; do sleep 1; timeout=$((timeout - 1)); done
